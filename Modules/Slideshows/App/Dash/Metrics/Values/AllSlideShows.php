...
namespace Modules\SlideShows\App\Dash\Metrics\Values;

use Dash\Extras\Metrics\Value;
use Modules\SlideShows\App\Models\SlideShow;

class AllSlideShows extends Value
{
    ...
    {
        return
            $this->count(SlideShow::class)
            ->column(4)
            ->href(dash('resource/SlideShows'))
            ->icon('<i class="fa-solid fa-chart-column"></i>')
            ->title(__('dash.SlideShows'))
