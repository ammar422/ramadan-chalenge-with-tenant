...
namespace Modules\Socials\App\Dash\Resources;

use Dash\Resource;
use Modules\Socials\App\Models\Social;
use Modules\Socials\App\Policies\SocialPolicy;
...
class Socials extends Resource
{
	 ...
	 * @var string $policy
	 */
	public static $policy = SocialPolicy::class;
	 ...
	 * @var string $group
	 */
	public static $group = 'Settings.Socials';
	...
	{
		return [
			text(__('socials::main.name'), 'name')
				->column(6)
				->rule('required', 'string', 'min:3', 'max:255'),

			uri(__('socials::main.url'), 'url')
				->column(6)
				->rule('required', 'active_url'),

			image(__('socials::main.icon'), 'icon')
				->deletable(true)
				->disableDownloadButton()
				->rule('nullable', 'file', 'mimes:png,jpg,jpeg,svg')
